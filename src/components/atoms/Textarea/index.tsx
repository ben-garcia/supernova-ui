...
import { createClasses, isFunction, isString, mergeRefs } from '@utils';
...
const Textarea = forwardRef((props: TextareaProps, ref: any) => {
  ...
  const addTextareaClasses = useCreateClassString('snui snui-textarea', {
    ...
    'snui-disabled': isDisabled,
    [`snui-textarea--${variant}`]: isString(variant) && !props.backgroundColor,
  ...
  const handleKeyUp = useCallback((e: KeyboardEvent<HTMLTextAreaElement>) => {
    if (textareaRef?.current) {
      ...
      textareaRef.current.style.height = isString(props.height)
        ? props.height
        : 'auto';
      textareaRef.current.style.height = `${(e.target as any).scrollHeight}px`;
    }
  }, []);
